...
def run_mcor(input_dict):
    ...
    optim.plot_best_system(scenario_criteria=dispatch_plots_settings_inputs["scenario_criteria"],
                           scenario_num=dispatch_plots_settings_inputs["scenario_num"])

    # Save results
    optim.save_results_to_file(spg, load_inputs["save_filename"])
    pickle.dump(optim, open('output/{}.pkl'.format(load_inputs["save_filename"]), 'wb'))
...
if __name__ == "__main__":
    ...
    ###########################################################################
    # commented out the old version of code (lines 137-352)
    ###########################################################################
